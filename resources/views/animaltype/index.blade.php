<x-layouts.app>
    ...
    <div class="flex flex-col gap-6 items-center justify-center min-h-screen bg-gray-100">
        <div class="px-8 py-6 mt-4 text-left shadow-lg bg-gray-200">
            <h1 class="text-3xl font-bold mb-[8px] md:mb-6 font-serif tracking-wider text-blue-700"> CREATE A Animal Type</h1>
    <form class="bg-gray-100 flex justify-center w-full " action="{{ route('animaltype.index') }}">
    ...
    <tbody class=" w-full px-10 ">
        @foreach ($animaltypes as $animaltype)
            <tr class="border-4 border-gray w-full flex flex-row justify-around items-center">
                <td class="justify-center  h-14 w-full items-center text-center flex">{{ $tanimaltype['name'] }}</td>
               


                <td class="justify-center w-full flex items-center"><a href="animaltype/edit/{{ $animaltyp['id'] }}"
                ...
                <td class="justify-center  w-full items-center flex">
                    <form action="/animaltype/{{ $animaltype['id'] }}" method="POST" class="flex justify-center items-center">
